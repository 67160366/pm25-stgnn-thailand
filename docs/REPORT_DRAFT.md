...
**คุณค่าที่พิสูจน์ได้ของระบบคือความสามารถระบุแหล่งกำเนิดที่อธิบายได้** ซึ่ง persistence และโมเดลที่ไม่ใช้กราฟทำไม่ได้: บนข้อมูล **held-out ปี 2568** ที่สถานีชายแดนแม่ฮ่องสอน (16 ก.พ. 2568) ระบบระบุว่าฝุ่นมาจากเมียนมาประมาณ 37% สอดคล้องกับสัดส่วนไฟข้ามแดนที่เชื่อมถึงสถานีจริง (37.3%) ขณะที่เหตุการณ์หมอกควันกลางเมืองเชียงใหม่ มีนาคม 2567 (141–144 µg/m³) ระบบระบุว่าเป็นการเผาในไทยเป็นหลัก (~100%; ค่า FRP ของไฟในไทยสูงกว่าเมียนมา 128 เท่า) แสดงให้เห็นว่าระบบจับการลำเลียงข้ามแดนได้จริง "เมื่อและที่ใด" มันเกิดขึ้น นอกจากนี้คณะผู้พัฒนายังตรวจพบและแก้ไขข้อผิดพลาดการ denormalize ในขั้นตอนรายงานผลด้วยตนเอง และเปิดเผยผลตามจริงทั้งหมด ซึ่งสะท้อนความเข้มงวดทางวิศวกรรมและธรรมาภิบาล AI ระบบทั้งหมดแสดงผลผ่าน dashboard (Streamlit) เพื่อสนับสนุนการตัดสินใจเชิงนโยบายสาธารณสุขและสิ่งแวดล้อม
...
- **สถานีตรวจวัด:** 18 สถานี Air4Thai (15 สถานีหลัก + 3 สถานีเสริม ตามเกณฑ์ใน docs/DESIGN.md) ที่มีข้อมูลครบช่วง 2565–2568
...
**(1) การสร้างกราฟพลวัต 3 ชนิดเส้นเชื่อม** — ระบบสร้างกราฟที่มี 18 station nodes และ M hotspot cluster nodes เชื่อมด้วยเส้นเชื่อม 3 ประเภท (รายละเอียดใน `docs/DESIGN.md` §5.2):
...
**(2) FIRMS hotspot เป็นโหนดต้นทางไฟ** — ข้อมูลไฟ VIIRS/MODIS จาก NASA FIRMS ถูกจัดกลุ่มเชิงพื้นที่เป็น hotspot clusters และเพิ่มเป็นโหนดพิเศษในกราฟ แต่ละโหนดเก็บ FRP รวมและพิกัดศูนย์กลาง (`[total_frp, centroid_lat, centroid_lon]`) พร้อมป้ายประเทศต้นทาง ซึ่งเป็นพื้นฐานของการระบุแหล่งกำเนิดเชิงประเทศ
...
**(ก) เหตุการณ์กลางเมืองเชียงใหม่ มีนาคม 2567 (141–144 µg/m³)** — แหล่ง `outputs/attribution_march2024.json`: ระบบระบุว่าฝุ่นมาจากการเผาในไทยเป็นหลัก (**Thailand ~100%, Myanmar 0%**) ซึ่งสอดคล้องกับข้อมูลดาวเทียม โดยค่า FRP รวมของไฟในไทย (109,564) สูงกว่าเมียนมา (854) ถึง **128 เท่า** ช่อง hotspot เพิ่มค่าพยากรณ์เฉลี่ย **3.49 µg/m³** (ช่วง 2.5–5.6) เหนือ baseline ที่ไม่มีไฟ และ Integrated Gradients ชี้ว่าปัจจัยความชื้น/อุณหภูมิ (d2m 0.0345, t2m 0.0344) มีผลเด่นกว่าปัจจัยอื่น เมื่อเทรนใหม่บน split ที่ทำให้ มี.ค. 2567 เป็น held-out ผลยังคงเป็น Thailand ~100% (`attribution_march2024_val.json`) ยืนยันว่าไม่ใช่การจดจำข้อมูล (memorization)

**(ข) การระบุข้ามแดนบนข้อมูล held-out 2568 ที่สถานีชายแดนแม่ฮ่องสอน** — แหล่ง `outputs/transboundary_attr_test_split2.json`: ในวันที่ 16 ก.พ. 2568 ไฟที่เชื่อมถึงสถานีเป็นของเมียนมา **37.3%** และระบบระบุว่ามาจากเมียนมา **36.6%** — สอดคล้องกับสัดส่วนไฟจริงอย่างดี ความสามารถระบุข้ามแดนนี้เกิดในทั้งโมเดลหลัก (`transboundary_attr_test.json` ให้ 33.2% วันเดียวกัน) และโมเดลรายงาน จึงไม่ใช่ของโมเดลเดียว อย่างไรก็ตาม **ขนาดของสัดส่วนขึ้นกับโมเดลและเหตุการณ์** (เช่น 25 มี.ค. 2568 โมเดลหลักขยายเป็น 100% ขณะที่โมเดลรายงานให้ ~4%) จึงต้องรายงานพร้อม caveat
