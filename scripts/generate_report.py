...
body(
    ...
    "ที่ไม่ใช้กราฟทำไม่ได้: บนข้อมูล **held-out ปี 2568** ที่สถานีชายแดนแม่ฮ่องสอน (16 ก.พ. 2568) "
    "ระบบระบุว่าฝุ่นมาจากเมียนมาประมาณ 37% สอดคล้องกับสัดส่วนไฟข้ามแดนที่เชื่อมถึงสถานีจริง (37.3%; "
    "ทั้งนี้ขนาดของสัดส่วนขึ้นกับโมเดลและเหตุการณ์ ดูรายละเอียดในผลการทดสอบ) ขณะที่เหตุการณ์หมอกควัน"
    "กลางเมืองเชียงใหม่ มีนาคม 2567 (141–144 µg/m³) ระบบระบุว่าเป็นการเผาในไทยเป็นหลัก (~100%; ค่า FRP "
    "ของไฟในไทยสูงกว่าเมียนมา 128 เท่า) แสดงให้เห็นว่าระบบจับการลำเลียงข้ามแดนได้จริง “เมื่อและที่ใด” "
...
body(
    ...
    "station (2025-02-16) the model attributes ~37% of pollution to Myanmar, matching the ~37% "
    "foreign share of connected fire activity, whereas the interior Chiang Mai March-2024 episode "
    "is attributed to predominantly Thai burning (~100%; Thai FRP 128× Myanmar's). We additionally "
...
body(
    ...
    "(`[total_frp, centroid_lat, centroid_lon]`) พร้อมป้ายประเทศต้นทาง ซึ่งเป็นพื้นฐานของการระบุแหล่ง"
    "กำเนิดเชิงประเทศ",
)
body(
    doc,
...
body(
    ...
    "`attribution_march2024.json`: ระบบระบุว่าฝุ่นมาจากการเผาในไทยเป็นหลัก (**Thailand ~100%, Myanmar "
    "0%**) ซึ่งสอดคล้องกับข้อมูลดาวเทียม โดยค่า FRP รวมของไฟในไทย (109,564) สูงกว่าเมียนมา (854) ถึง "
    "**128 เท่า** ช่อง hotspot เพิ่มค่าพยากรณ์เฉลี่ย **3.49 µg/m³** (ช่วง 2.5–5.6) เหนือ baseline ที่ไม่มี"
    ...
    "(`attribution_march2024_val.json`) ยืนยันว่าไม่ใช่การจดจำข้อมูล (memorization)",
)
body(
    doc,
    "**(ข) การระบุข้ามแดนบนข้อมูล held-out 2568 ที่สถานีชายแดนแม่ฮ่องสอน** — แหล่ง "
    "`transboundary_attr_test_split2.json`: ในวันที่ 16 ก.พ. 2568 ไฟที่เชื่อมถึงสถานีเป็นของเมียนมา "
    "**37.3%** และระบบระบุว่ามาจากเมียนมา **36.6%** — สอดคล้องกับสัดส่วนไฟจริงอย่างดี ความสามารถระบุ"
    "ข้ามแดนนี้เกิดในทั้งโมเดลหลัก (`transboundary_attr_test.json` ให้ 33.2% วันเดียวกัน) และโมเดล"
    "รายงาน จึงไม่ใช่ของโมเดลเดียว อย่างไรก็ตาม **ขนาดของสัดส่วนขึ้นกับโมเดลและเหตุการณ์** (เช่น 25 "
    "มี.ค. 2568 โมเดลหลักขยายเป็น 100% ขณะที่โมเดลรายงานให้ ~4%) จึงต้องรายงานพร้อม caveat",
)
body(
    doc,
...
figure(
    ...
    "รูปที่ 6 จุดความร้อนใกล้สถานีแม่ฮ่องสอน (16 ก.พ. 2568) แยกสีตามประเทศ — "
    "ไฟเมียนมาอยู่ทางทิศตะวันตกของสถานี",
    width_cm=12.0,
)
figure(
    doc,
